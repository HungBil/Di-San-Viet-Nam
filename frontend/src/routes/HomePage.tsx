...
const featureStrip = [
  {
    title: "Tham quan 3D",
    text: "Không gian di tích được tái hiện có chiều sâu",
  },
  {
    title: "Thuyết minh AI",
    text: "Trò chuyện và tra cứu theo từng câu chuyện lịch sử",
  },
  {
    title: "Dấu tích lịch sử",
    text: "Theo mạch sự kiện, nhân vật và địa danh",
  },
        ...
        <div className="relative z-20 mx-auto flex h-full max-w-7xl items-center px-4 py-10 sm:px-6 lg:px-8">
          <div className="flex w-full max-w-2xl flex-col justify-center py-6 lg:max-w-[590px]">
            ...
            <p className="mt-5 max-w-xl text-lg leading-8 text-[var(--heritage-ink)]">
              Hành trình số hóa bảo tàng, di tích và ký ức văn hóa Việt Nam qua
              bản đồ, không gian 3D và thuyết minh AI.
            </p>
            ...
            <div className="mt-7 grid gap-4 border-t border-[var(--heritage-line)] pt-5 sm:grid-cols-2 xl:grid-cols-4">
              {featureStrip.map((item) => (
                <div key={item.title} className="flex gap-3">
                  <span>
                    <span className="block text-sm font-semibold text-[var(--heritage-ink)]">
                      {item.title}
                    </span>
                    <span className="mt-1 block text-xs leading-5 text-[var(--heritage-muted)]">
                      {item.text}
                    </span>
                  </span>
                </div>
      ...
      <div className="relative">
          ...
          <div className="relative z-10 mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
            <div className="flex flex-col justify-between gap-4 border-b border-[var(--heritage-line)] pb-4 sm:flex-row sm:items-end">
              <div className="flex gap-8 items-end">
                <div>
                  <p className="text-xs uppercase tracking-normal text-[var(--heritage-muted)]">
                    Dấu tích và địa danh
                  </p>
                  <h2 className="mt-2 font-serif text-3xl text-[var(--heritage-brown)]">
                    Điểm đến nổi bật
                  </h2>
                </div>
                ...
                >
                  ...
                  />
                  <button
                    className="grid w-10 place-items-center border-l border-[var(--heritage-line)] text-[var(--heritage-brown)]"
                    aria-label="Tìm kiếm"
                  >
                    <Search size={22} />
                  </button>
              ...
              <Link
                to="/map"
                className="inline-flex items-center gap-1 text-sm text-[var(--heritage-muted)] transition hover:text-[var(--heritage-bronze)]"
              >
                Xem tất cả
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-6">
              {featuredPlaces.map((place) => (
                ...
                >
                  <img
                    className="heritage-image absolute inset-0 h-full w-full object-cover transition duration-300 group-hover:scale-[1.04]"
                    src={place.image}
                    alt={place.name}
                  />
        ...
        <section className="mx-auto flex max-w-7xl flex-col gap-4 px-4 pb-10 sm:px-6 lg:h-[410px] lg:flex-row lg:items-start lg:px-8">
          <div className="relative min-h-[380px] overflow-hidden rounded-lg border border-[var(--heritage-line)] bg-[var(--heritage-ink)] text-[var(--heritage-white)] shadow-[0_18px_36px_rgba(45,40,32,0.16)] lg:h-full lg:min-h-0 lg:w-[580px] lg:flex-none xl:w-[600px]">
            <img
              className="heritage-image absolute inset-0 h-full w-full object-cover opacity-75"
              src={heroImage}
              alt="Tham quan 3D Văn Miếu"
            />
            <div className="absolute inset-0 bg-gradient-to-t from-[#2d2820] via-[#2d2820]/28 to-transparent" />
            <div className="relative p-6">
              <p className="text-xs uppercase tracking-normal text-[var(--heritage-paper-deep)]">
                Không gian số
              </p>
              <h2 className="mt-2 font-serif text-4xl text-[var(--heritage-paper-light)]">
                Tham quan 3D
              </h2>
              <p className="mt-2 text-base text-[rgba(248,241,227,0.82)]">
            ...
            <div className="absolute bottom-6 left-6 right-6 flex items-end justify-between gap-4">
              <div className="flex gap-2">
                {[
                  heroImage,
                  ...featuredPlaces.slice(0, 4).map((place) => place.image),
                ].map((image, index) => (
                  <img
                    key={`${image}-${index}`}
                    className="heritage-image h-14 w-20 rounded border border-[var(--heritage-line)] object-cover"
                    src={image}
                    alt=""
                  />
                ))}
        ...
        <section className="mx-auto grid max-w-7xl gap-6 px-4 pb-14 sm:px-6 lg:grid-cols-[1fr_380px] lg:px-8">
          <div>
            ...
            <h2 className="mt-2 font-serif text-3xl text-[var(--heritage-brown)]">
              Vì sao nên khám phá di sản Việt online?
            </h2>
              ...
              <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
                {benefits.map((item) => (
                  <div key={item.title} className="flex items-center gap-3">
                    ...
                    <p className="text-sm leading-5 text-[var(--heritage-ink)]">
                      {item.title}
                    </p>
                  </div>
                ))}
          ...
          <div className="overflow-hidden rounded-lg border border-[var(--heritage-line)] bg-[var(--heritage-paper-deep)] shadow-[0_18px_36px_rgba(45,40,32,0.12)]">
            <div className="grid grid-cols-[1fr_150px]">
              <div className="p-5">
                <h3 className="font-serif text-2xl text-[var(--heritage-brown)]">
                  Bạn có câu chuyện về di sản?
                </h3>
                <p className="mt-3 text-sm leading-6 text-[var(--heritage-muted)]">
                  Chia sẻ hình ảnh, câu chuyện và ký ức địa phương để cùng gìn
                  giữ di sản Việt Nam.
                </p>
                <Link
                  to="/map"
                  className="mt-4 inline-flex items-center gap-2 rounded-md bg-[var(--heritage-brown)] px-4 py-2 text-sm font-semibold text-[var(--heritage-white)]"
                >
                  <Share2 size={16} />
                  Đóng góp ngay
                </Link>
              ...
              <img
                className="heritage-image h-full min-h-44 object-cover"
                src="/images/bao-tang-lich-su.webp"
                alt="Chia sẻ di sản"
              />
            </div>
          </div>
